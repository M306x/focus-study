...
const STORAGE_KEY = 'study_dashboard_data_v1';
const THEME_KEY = 'study_theme_preference'; // novo
...
export default function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(true); // padrão dark como era antes
  ...
  // --- resto do useEffect de persistência permanece igual ---
  useEffect(() => {
    const savedData = localStorage.getItem(STORAGE_KEY);
    if (savedData) {
      try {
  ...
  // ... (manter todos os outros useEffect e funções exatamente iguais)

  // Função auxiliar para alternar tema
  ...
  return (
    ...
    >
      ...
      {/* ANIMAÇÃO DE BREAK — mantida apenas no dark mode por estética */}
      ...
      {/* NAVEGAÇÃO SUPERIOR */}
      <header className={`
        h-20 border-b flex items-center justify-between px-12 shrink-0 z-10 transition-colors
        ${isDarkMode 
      ...
      `}>
        <div className="flex items-center gap-3">
          ...
          <span className={`
            font-bold tracking-tighter text-lg uppercase
        ...
        <nav className="flex gap-4">
          {[
            ...
            <button
              ...
              onClick={() => setView(item.id)}
              className={`
                flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-[10px] font-bold uppercase tracking-widest
                ${view === item.id 
                  ? isDarkMode 
      ...
      `}>
        <div className="p-8 max-w-5xl mx-auto pb-24">
          {/* 
            Aqui você mantém todo o conteúdo das views (focus, labels, dashboard, goals, settings)
            Apenas adicionei classes condicionais dark/light onde necessário.
            Vou mostrar apenas alguns exemplos representativos — aplique o padrão no resto.
          */}

          {view === 'focus' && (
            <div className="flex flex-col items-center justify-center pt-8">
              <div className={`
                flex flex-wrap justify-center gap-2 p-1.5 rounded-2xl mb-12 border
                ${isDarkMode 
              ...
              {/* ... continua o restante do focus view ... */}
              {/* Ajuste principalmente textos, fundos e botões */}
              {/* Exemplo do timer principal: */}
              <button
                onClick={() => { if (!isRunning) { setTempInputValue(customTime.toString()); setModalType('editTime'); } }}
                className={`
                  text-[10rem] md:text-[12rem] font-light tracking-tighter tabular-nums leading-none cursor-pointer transition-all hover:opacity-80
                  ${mode === 'break' 
              ...
              {/* ... ajuste botões Play/Pause, etc. seguindo o mesmo padrão ... */}
            </div>
          )}

          {/* 
            Faça o mesmo para as outras views:
            - labels → fundos → bg-white / border-zinc-200 no light
            - dashboard → cards → bg-white border-zinc-200 shadow-sm no light
            - gráficos → ajuste cores de fundo e texto
            - settings → seções → bg-white/70 border-zinc-200 no light
          */}
          ...
          {view === 'dashboard' && (
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className={`
                  p-6 rounded-[2rem] flex items-start gap-4 border
                  ${isDarkMode 
              ...
              {/* ... ajuste os gráficos de calor, barras etc. */}
              {/* Para o heat calendar no light mode: */}
              {/* backgroundColor: hasStudy ? `rgba(${baseColor}, ${intensity})` : 'rgba(200,200,200,0.3)'  */}
            </div>
          )}
          ...
          {modalType === 'editTime' && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
              <div className={`
                p-8 rounded-[2rem] w-full max-w-xs text-center border
                ${isDarkMode 
                  ? 'bg-zinc-900 border-zinc-800' 
                  : 'bg-white border-zinc-200 shadow-2xl'}
              `}>
                <h3 className={`
                  font-bold mb-6 uppercase text-[10px] tracking-widest opacity-60
                ...
                <input
                  ...
                  className={`
                    w-full border rounded-2xl p-6 mb-6 text-center outline-none font-bold text-5xl tracking-tighter
                    ${isDarkMode 
          ...
          {/* Ajuste os outros modais e seções de forma similar */}
        </div>
      </main>
    </div>
  );
